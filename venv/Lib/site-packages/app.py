...
#agent= Agent.load(".\models\20240126-182354-ivory-iteration.tar.gz")
...
with placeholder.container():
    ...
    if st.button("Add to database"):
        check=collection.find_one({"Name":name})
        if not check:
            ...
            st.error("company already exist")
    ...
    if st.button("Delete Company"):
        ...
        res=collection.delete_one(data_to_delete)
        if res.deleted_count!=0:
            st.success("Data Deleted")
            os.system("streamlit run app.py")
        else:
            st.error("There's no company with the Name "+name_to_delete)

    #Chatbot
    """st.title("RASA chatbot")
    user_inupt=st.text_input("You:","")
    if st.button("Send"):
        response=agent.handle_text(user_inupt)
